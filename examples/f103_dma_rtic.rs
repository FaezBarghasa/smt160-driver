...
//! SMT160 DMA-based High Precision RTIC Example
//!
//! # Expected Resource Usage
//! - **Flash**: ~6KB (Release build with minimal panic handler and standard `no_std` config)
//! - **RAM**: ~1KB (Mainly RTIC overhead, `[u16; 64]` DMA buffer takes exactly 128 bytes)
//! 
//! # Architecture
//! Producer: DMA1 Channel 7 automatically captures `TIM2->CCR1` (Period) and `TIM2->CCR2` (Active)
//! into a circular RAM buffer (`[u16; 64]`) without CPU intervention.
//! Consumer: RTIC task wakes up on DMA Half/Full events to batch-process 32 samples (16 pairs).
...
mod app {
    ...
    fn init(cx: init::Context) -> (Shared, Local) {
        ...
        // Setup RCC with External Crystal (HSE) at 72MHz for highest timer resolution
        ...
        // Configure DMA burst: DBA = CCR1 (offset 13 from CR1), DBL = 1 (2 transfers: CCR1, CCR2)
    ...
    fn dma1_ch7_irq(cx: dma1_ch7_irq::Context) {
        ...
        if let Some(range) = process_range {
            ...
            for i in 0..16 {
                ...
                // Pack into standard DMA format: (Period << 16) | Active
